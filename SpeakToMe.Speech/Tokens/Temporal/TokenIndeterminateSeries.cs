...
namespace SpeakToMe.Speech.Tokens.Temporal
{
    ...
    public class TokenIndeterminateSeries : Token, IParseToken
    {
        ...
        public TokenIndeterminateSeries()
        {
            _tokens = LoadTokens();
        ...
        public override IEnumerable<TokenResult> Parse(string input, string UserId)
        {
            ...
            TokenIndeterminateSeries theSeries;
            ...
            foreach (var temporalRule in _temporalRules)
            {
                ...
                if (parms.Count == parameters.Count())
                {
                    ...
                    theSeries = (TokenIndeterminateSeries)temporalRule.Invoke(null, passins);
                    ...
                    if (theSeries != null)
                    {
                        ...
                        returnResults.Add(new TokenResult
                        {
                            Start = start,
                            Length = length,
                            TokenType = typeof(TokenIndeterminateSeries).ToString(),
                            Token = theSeries,
                            Value = theSeries
                        });
        ...
        private static List<MethodInfo> LoadRules()
        {
            ...
            var methods = from m in ruleClass.GetMembers()
                          where m.Name == "GetIndeterminateSeriesFromTokens"
                          select m;
        ...
        public override string ToString()
        {
            ...
            {
                response = string.Format("every {0}", this.StartDate.DayOfWeek.ToString());
            }
            else
            {
